...
6. Click on the link. You should see outbound traffic correlated with the npm dependencies. StepSecurity has published a test package `@step-security/malware-simulator` that makes an outbound call as a `preinstall` step to simulate a malicious outbound call. You can update the workflow using the recommended policy to block other outbound calls.
